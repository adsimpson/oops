module Oops
  ...
  module ControllerAdditions
    ...
    included do
      rescue_from Oops::BaseError, :with => :render_error
      class_attribute :error_mapping
      self.error_mapping = {}
      
      include Oops::ActiveRecordErrors
      include Oops::ActionControllerErrors
    end
    ...
    def routing_error!
      raise ActionController::RoutingError.new("Route not found: #{params[:unmatched_route]}")
    end
    ...
    def lookup_error_message(exception)
      ...
      context = lookup_error_context(exception).reverse_merge(:scope => :"oops", :default => message)
      I18n.t name, context.except(:metadata)
    end
    
    def default_message_for_exception(exception)
      ##if !Oops.show_exception_message? || exception.message == exception.class.name
    ...
    def render_error(exception)
      ...
      http_status = lookup_error_status(exception)
      error_info = {
        :status => http_status,
        :type => lookup_error_name(exception).to_s,
        :message => lookup_error_message(exception)
      }.merge(lookup_error_metadata(exception))
          
      render :json => { :error => error_info }, :status => http_status
